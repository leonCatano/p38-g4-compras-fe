    ...
    <div>
      <table class="table">
        ...
        <tr v-for="creditCardByUserId in creditCardByUserId" :key="creditCardByUserId.id" >
          ...
          <td>
            <button v-on:click="deleteCreditCard(creditCardByUserId.id)" style="cursor: pointer; font-size: 10px; padding: 5px 8px;  border-radius: 50%;  background: #ff0018a8"><strong>X</strong></button>
          </td>
        </tr>
      </table>
    </div>
...
deleteCreditCard: async function(creditCardId){
                await this.$apollo.mutate(
                    {
                        mutation: gql`
